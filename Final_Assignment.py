...
# Locate the table with Tesla Quarterly Revenue by checking column names
tesla_revenue = None
for table in tables:
    if table.shape[1] == 2:  # Check if the table has 2 columns
        tesla_revenue = table
        break
...
ticker_symbol1 = 'GME'
stock = yf.Ticker(ticker_symbol1)
...
# Locate the table with GME Quarterly Revenue by checking column names
gme_revenue = None
for table in tables:
    if table.shape[1] == 2:  # Check if the table has 2 columns
        gme_revenue = table
...
if gme_revenue is not None:
    ...
    print("Table with GME Quarterly Revenue not found.")
...
def make_graph(stock_data, revenue_data, stock):
    ...
    revenue_data_specific = revenue_data[pd.to_datetime(revenue_data['Date']) <= pd.to_datetime('2021-04-30')]
    fig.add_trace(go.Scatter(x=pd.to_datetime(stock_data_specific['Date'], infer_datetime_format=True), y=stock_data_specific['Close'].astype("float"), name="Share Price"), row=1, col=1)
    fig.add_trace(go.Scatter(x=pd.to_datetime(revenue_data_specific['Date'], infer_datetime_format=True), y=revenue_data_specific['Revenue'].str.replace(',', '').str.replace('$', '').astype("float"), name="Revenue"), row=2, col=1)
    fig.update_xaxes(title_text="Date", row=1, col=1)
    fig.update_xaxes(title_text="Date", row=2, col=1)
    fig.update_yaxes(title_text="Price ($US)", row=1, col=1)
    fig.update_yaxes(title_text="Revenue ($US Millions)", row=2, col=1)
...
# Call the make_graph function with the given data
make_graph(tesla_data.reset_index(), tesla_revenue, 'Tesla')
...
def make_graph(stock_data, revenue_data, stock):
    ...
    # Convert the 'index' column to datetime type
    stock_data.index = pd.to_datetime(stock_data.index)
    revenue_data['Date'] = pd.to_datetime(revenue_data['Date'])

    stock_data_specific = stock_data[stock_data.index <= pd.to_datetime('2021-06-14')]
...
def make_graph(stock_data, revenue_data, stock):
    ...
    fig.write_image("stock_graph.png")  # Change the extension to '.jpeg' if you prefer JPEG format
